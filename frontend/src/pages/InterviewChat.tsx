...
const handleSend = async () => {
  ...
  try {
    ...
    setInterview((prev: any) => {
      ...
      return {
        ...prev!,
        status: res.data.is_finished ? 'completed' : 'in_progress',
        current_round: res.data.current_round,
        messages: newMessages,
        total_score: res.data.is_finished
          ? (res.data.total_score ?? prev?.total_score ?? 0)
          : prev?.total_score,
        evaluation: res.data.is_finished
          ? (res.data.evaluation ?? prev?.evaluation ?? {})
          : prev?.evaluation,
      };
    });
  } finally {
      ...
      {!isFinished && (
        <div className="flex gap-2">
          <textarea
            ...
            placeholder="输入你的回答..."
            className="flex-1 border rounded-xl px-4 py-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-300"
            rows={1}
          ...
          <button
            ...
            className="bg-gradient-to-r from-blue-100 to-sky-100 text-blue-600 rounded-xl px-6 py-3 text-sm font-medium border border-blue-200 disabled:opacity-50 hover:from-blue-200 hover:to-sky-200 transition-all"
          >
            {sending ? '思考中...' : '发送'}
          </button>
        </div>
